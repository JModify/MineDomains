...
/**
 * Template loader is a slime loader used for copying theme/template files
 * from the plugin's data directory. These files are always cloned, then stored
 * into mongodb.
 */
public class SlimeTemplateLoader implements SlimeLoader {
    ...
    private final PlugDebugger debugger;

    public SlimeTemplateLoader() {
        this.templateDir = new File(MineDomains.getInstance().getDataFolder() + "/themes/");
        this.debugger = MineDomains.getInstance().getDebugger();
    }

    @Override
    public byte[] loadWorld(String worldName, boolean readOnly) throws UnknownWorldException, IOException {
        PlugDebugger debugger = MineDomains.getInstance().getDebugger();
